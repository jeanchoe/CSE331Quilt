...
import { RED, Square, Row, rnil, qnil, Quilt } from './quilt';
...
export const RowTableElems = (props: {row: Row, key: number}): JsxList => {
  if(props.row === rnil){
    ...
    const head = SquareTableElem({ square: props.row.hd, key: props.key });
    const tail = RowTableElems({ row: props.row.tl, key: props.key + 1 });
    return jcons(head, tail);
  }
}
...
export const QuiltTableElems = (props: {quilt: Quilt, key: number}): JsxList => {
  if(props.quilt === qnil){
    ...
    const head = RowTableElem({ row: props.quilt.hd, key: props.key });
    const tail = QuiltTableElems({ quilt: props.quilt.tl, key: props.key + 1 });
    return jcons(head, tail);
  }
}
